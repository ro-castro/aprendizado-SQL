/* 1. Para fins fiscais, a contabilidade da empresa precisa de uma tabela contendo todas as vendas referentes à loja ‘Contoso Orlando Store’. Isso porque essa loja encontra-se em uma região onde a tributação foi modificada recente. Portanto, crie uma consulta ao Banco de Dados para obter uma tabela FactSales contendo todas as vendas desta loja. */
	...
	UnitPrice,
		...
		SELECT
			...
			FROM
				DimProduct
			WHERE
				ProductKey = 1893) AS 'Preço produto 1893'
FROM
	DimProduct
WHERE
	UnitPrice > (
...
/*3. A empresa Contoso criou um programa de bonificação chamado “Todos por 1”. Este programa consistia no seguinte: 
1 funcionário seria escolhido ao final do ano como o funcionário destaque, só que a bonificação seria recebida por todos da área daquele funcionário em particular. 
...
DECLARE
	@nome varchar(30) = 'Miguel Severino'
SELECT
	CONCAT(FirstName, ' ', LastName) AS 'Nome e Sobrenome',
	DepartmentName
FROM
	DimEmployee
WHERE
	DepartmentName = (
		...
		FROM
			DimEmployee
		WHERE
			CONCAT(FirstName, ' ', LastName) = 'Miguel Severino'
	)
ORDER BY
...
WHERE
	CustomerKey IN (
		...
		GROUP BY
			...
			ProductKey
		HAVING
			COUNT(SalesQuantity) >= 3000
	)

/*7. Você deverá criar uma consulta para o setor de vendas que mostre as seguintes colunas da tabela DimProduct: 
ProductKey, 
ProductName, 
BrandName, 
UnitPrice 
...
SELECT
	...
	ROUND((
		SELECT
			AVG(UnitPrice)
		FROM
			DimProduct), 2) AS 'Média de Preço'
FROM
	DimProduct
...
/*9. Crie uma CTE que seja o agrupamento da tabela DimProduct, armazenando o total de produtos por marca. Em seguida, faça um SELECT nesta CTE, descobrindo qual é a quantidade máxima de produtos para uma marca. Chame esta CTE de CTE_QtdProdutosPorMarca. */

GO
WITH CTE_QtdProdutosPorMarca (Marca, Quantidade) AS (
	SELECT
		BrandName,
		COUNT(ProductKey)
	FROM
...
(i) a primeira deve conter as colunas ProductKey, ProductName, ProductSubcategoryKey, BrandName e UnitPrice, da tabela DimProduct, mas apenas os produtos da marca Adventure Works.
Chame essa CTE de CTE_ProdutosAdventureWorks. 

(ii) a segunda deve conter as colunas ProductSubcategoryKey, ProductSubcategoryName, da tabela DimProductSubcategory mas apenas para as subcategorias ‘Televisions’ e ‘Monitors’. 
...
FROM
	CTE_ProdutosAdventureWorks
INNER JOIN
--LEFT JOIN -> O LEFT JOIN traz também os produtos AdventureWorks que nao são Televisions nem Monitors
	CTE_CategoriaTelevisionsERadio
ON
	CTE_ProdutosAdventureWorks.ProductSubcategoryKey = CTE_CategoriaTelevisionsERadio.ProductSubcategoryKey
